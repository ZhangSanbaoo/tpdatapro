...
object ClimateService {
  ...
  def showCO2Data(list: List[Option[CO2Record]]): Unit = {
    ...
    val validRecords = list.flatten
    logger.info(s"Valid records: ${validRecords.size}")
    val invalidRecords = list.count(_.isEmpty)
    logger.info(s"Invalid records: $invalidRecords")
    validRecords.map(_.show).foreach(logger.info)
  }
